...
> **Depends on:** Blueprint v5.1 authoritative + SEO Technical Contract v1.1  
...
To stop new drift as early as possible:

```text
P0-T7 AI SEO safety hotfix
P0-T4 Claims / positioning cleanup
P0-T3 Review verification integrity
P0-T5 Testing evidence
...
Tasks may overlap where technically safe, but issue/commit naming must use `P0-T1...P0-T7`.  
`P0.5` is reserved exclusively for the **SEO Technical Contract phase**.
...
- review existing `backend/app/Services/ProductRouteService.php` first;
- designate it as Product canonical resolver for P0/P1 unless review proves it structurally unsuitable;
...
### Known instance — `frontend/components/FaqsPage.tsx`

A partial, uncommitted fix already exists on `main` for one FAQ answer: `"Shop by Breed"` → `"Explore by Breed"`, `"Shop by Solution"` → `"Explore Solutions"`, `"Mobility & Support"` → `"Mobility"` were corrected, but the same sentence still references a `"Productivity"` collection, which is not a canonical Solution (only `feeding/comfort/mobility/walking` exist per Blueprint §5). The described behavior ("helps pets focus during mealtime") describes Feeding, not a nonexistent "Productivity" category — likely a leftover from a pre-v5 taxonomy. Fix this line as part of the full FAQ/taxonomy sweep, and check the rest of `FaqsPage.tsx` for other stale category labels while in there.
...
# 2A. P0.5 — DEFINE / FREEZE SEO TECHNICAL CONTRACT

Before P1 begins, freeze these decisions:

1. **Product canonical resolver owner:** `ProductRouteService`.
2. **Public SEO DTO:** one normalized Laravel DTO.
3. **JSON-LD ownership:** Laravel owns domain schema facts/evidence eligibility; Next.js owns final public graph composition.
4. **Public sitemap owner:** Next.js `/sitemap.xml`; Laravel supplies sitemap-oriented data.
5. **Indexability:** resolve from existing Product status + `seo.is_indexable` / `seo.is_followable`; no duplicate enum unless code review proves necessary.
6. **Metadata precedence:** contract v1.1.
7. **Product canonical override:** disabled in P0/P1; admin field hidden/read-only/unsupported.
8. **Canonical target rule:** canonical URL must return 200; alternate URLs redirect directly with no chain.

P1 may not start with any of these still ambiguous.
