...
> **Depends on:** `PetPosture-Canonical-Implementation-Blueprint-v5.1.md`  
...
Phải có một resolver/service duy nhất, ví dụ:
...
Không hard-code Product paths ở nhiều layer độc lập.
...
Admin canonical override chỉ được hỗ trợ nếu có validation rõ ràng; mặc định không cho override Product canonical tùy ý.
...
- published_noindex → accessible, `noindex, follow`, not in sitemap;
- published_indexable → index/follow, eligible for sitemap;
- private/account/cart/checkout/auth → noindex;
- transactional return/request forms → noindex unless explicitly justified;
- preview → noindex.

Không dùng `robots.txt Disallow` thay cho HTML/meta `noindex`.

---

# 10. SITEMAP CONTRACT

Chọn **một authoritative sitemap implementation**.

Sitemap phải:

- fetch all pages, including pagination;
