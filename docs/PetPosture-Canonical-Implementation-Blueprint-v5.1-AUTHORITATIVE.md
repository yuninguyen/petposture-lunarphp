# PETPOSTURE — CANONICAL IMPLEMENTATION BLUEPRINT v5.1
...
> **Supersedes:** `PETPOSTURE-BLUEPRINT-V5.md` and earlier conflicting PetPosture blueprint decisions.  
> **Migration note:** v5.1 preserves the v5 core architecture and adds SEO implementation invariants plus the official P0/P0.5/P1 execution roadmap.  
...
# 36. P0.5 IMPLEMENTATION OWNER DECISIONS

Các quyết định sau được chốt cho P0.5 và là đầu vào bắt buộc của P1.

## 36.1 Product canonical resolver owner

`backend/app/Services/ProductRouteService.php` là **Product canonical route resolver chính thức** trong phase P0/P1.

Không tạo thêm service thứ hai cùng tự build Product canonical URL.

P0 phải review `ProductRouteService` để xác nhận/hoàn thiện:

- canonical category;
- canonical slug;
- public storefront path;
- redirect history;
- route mismatch redirect;
- independence from mutable request context;
- unit-testability;
- reuse by ProductResource/API, sitemap data, Product cards, Related Products và Admin preview.

Nếu sau này cần façade chung cho nhiều entity, façade đó phải **delegate Product URL resolution về `ProductRouteService`**, không duplicate logic.

## 36.2 JSON-LD ownership

**Laravel owns domain schema facts.**

Laravel quyết định và trả dữ liệu domain/evidence đủ điều kiện:
...
- evidence/testing eligibility.

**Next.js owns public page composition.**

Next.js tạo JSON-LD graph cuối dựa trên:

- canonical public URL;
- page type;
- breadcrumbs;
- site/Organization references;
- domain schema facts từ Laravel.

Không để Laravel và Next.js cùng independently tính rating/eligibility.

## 36.3 Public sitemap owner
...
Laravel cung cấp sitemap-oriented data/DTO:
...
- ghép public `SITE_URL`;
- xuất sitemap/sitemap index;
- split sitemap khi cần;
- chỉ xuất canonical/indexable/HTTP-200 storefront URLs.

Backend sitemap public cũ phải được **retired hoặc permanent redirect** sau khi migration được xác nhận; không duy trì hai public sitemap owners cạnh tranh.

## 36.4 Product canonical override policy

Trong P0/P1:

- Product **không hỗ trợ custom canonical override**.
- Product canonical chỉ đến từ `ProductRouteService`.
- Canonical input hiện có trong Product admin phải read-only, hidden, hoặc ghi rõ unsupported/future.
- Không để admin control tồn tại nhưng storefront silently ignore.

Editorial Blog/Page có thể hỗ trợ canonical override theo contract riêng và validation chặt chẽ.

## 36.5 Product indexability resolution

Không tạo enum/migration mới nếu fields hiện tại đủ.
...
→ noindex
→ follow = seo.is_followable nếu policy cho phép
→ sitemap exclude

status == published && seo.is_indexable == true
→ index
→ follow = seo.is_followable
→ sitemap eligible nếu canonical URL hợp lệ và HTTP 200
```

Một resolver phải tạo state cuối; không để Product status và SEO flags trở thành hai nguồn quyết định độc lập.
