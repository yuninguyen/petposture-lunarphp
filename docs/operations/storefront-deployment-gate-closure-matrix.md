...
- Deployment gate closure now stands at: merge done (local, not pushed) + secrets provisioned. Remaining before deploy: push to `origin` (if desired), build/release with an immutable image digest (current tags are mutable `:prod`), and a named rollback owner/procedure — none of that has happened yet.
