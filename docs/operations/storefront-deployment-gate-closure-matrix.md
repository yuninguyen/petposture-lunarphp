...
- **`scripts/deploy-vps.sh` first real test run (2026-09-12)**, deploying `origin/main` (then `d2d6f87`) — exactly the caution above called for, and it found two real bugs in the script itself, not the application:
  1. **Container-name conflict.** `docker-compose.prod.yml` pins fixed `container_name` values; the running containers had been started under a different (or no) Compose project label by every prior manual deploy, so `docker compose -p petposture up -d --force-recreate` refused to touch them ("Conflict... already in use"). Neither container was disrupted and `DEPLOYED_COMMIT` correctly stayed unchanged (fail-safe worked as designed) — but the deploy itself didn't complete. Fixed (`d2d6f87`) by force-removing containers by name before recreating, since the names are already the real uniqueness constraint here.
  2. **Health-check false negative, silently.** `curl ... -w '%{http_code}' || echo 000` prints `000` from curl's own `-w` output on a connection failure *and* the `||` branch appends another `000`, producing `000000` — a string that never matches `"000"` in the guard, so a genuinely unreachable backend would have passed the check and the markers would have updated anyway. On the retry run backend was actually healthy by the time it was checked (confirmed independently via `docker ps`/`docker logs`/manual `curl`), so this didn't cause a real incident, but the check itself could not be trusted. Fixed (`424c8c8`) with a small retry loop that overwrites the status on failure instead of appending, and retries for ~18s to tolerate FrankenPHP/Laravel cold boot.
  - After both fixes, a clean re-run built and deployed successfully: `petposture-backend`/`petposture-frontend` recreated from `d2d6f87794...`, `DEPLOYED_COMMIT` updated correctly, backend logs show `storefront:refresh-replay` running every minute as expected, and a fresh cache-busted browser check of `https://petposture.com/` returned `200`.
- The deployment gate is **READY** for this branch's scope, and `scripts/deploy-vps.sh` is now real-world-tested (not just reviewed) as of `424c8c8` — its first two genuine defects were caught by actually running it against production and fixed in the same session, rather than assumed correct from reading it.
