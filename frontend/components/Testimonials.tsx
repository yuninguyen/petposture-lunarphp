    ...
    <section className="py-24 px-4 md:px-8 bg-white border-b border-zinc-50">
      <div className="container mx-auto max-w-6xl">
        <div className="flex flex-col items-center mb-16">
          <h2 className="text-center text-sm font-bold uppercase tracking-[0.4em] text-primary">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {reviews.map(({ title, text, author, rating, tag }, i) => (
            <div key={i} className="flex flex-col p-8 border border-zinc-100 hover:shadow-xl hover:border-secondary/20 transition-all duration-500 group">
              <span className="text-xs font-bold uppercase tracking-[0.2em] text-secondary mb-6">{tag}</span>
              
              <h4 className="text-sm font-black uppercase tracking-[0.1em] text-primary mb-4 leading-tight group-hover:text-secondary transition-colors">
                {title}
              </h4>
              
              <p className="text-md text-primary/60 leading-relaxed mb-10 flex-1">
                &ldquo;{text}&rdquo;
              </p>
