...
class BlogCategoriesAbilityParityTest extends TestCase
{
    ...
    {
        foreach (self::ALLOWED as $role) { $this->actAs($role); foreach ($this->requests() as [$method, $uri, $payload]) $this->assertNotSame(403, $this->{$method}($uri, $payload)->getStatusCode(), "{$role}: {$method} {$uri}"); }
    }

    public function test_blocked_roles_are_forbidden_from_every_blog_category_route(): void
    {
        foreach (self::BLOCKED as $role) { $this->actAs($role); foreach ($this->requests() as [$method, $uri, $payload]) $this->assertSame(403, $this->{$method}($uri, $payload)->getStatusCode(), "{$role}: {$method} {$uri}"); }
    }
    ...
    public function test_migration_assigns_only_underscore_abilities_to_core_roles(): void
    {
        ...
        foreach (self::ALLOWED as $role) foreach (self::ABILITIES as $ability) $this->assertTrue(Role::findByName($role)->hasPermissionTo($ability));
        foreach (self::BLOCKED as $role) foreach (self::ABILITIES as $ability) $this->assertFalse(Role::findByName($role)->hasPermissionTo($ability));
