...
class JournalOwnerReplayTest extends TestCase
{
    ...
    public function test_real_observer_snapshot_survives_dispatch_loss_and_scope_reset_then_replays(string $owner): void
    {
        ...
        $save = function () use ($setting) {
            $setting->update(['key' => 'new', 'value' => 'committed']);
        };
        if ($owner === 'non-http') {
            DB::transaction($save);
        } else {
        ...
        StorefrontHttp::assertPurgeCount($owner === 'initial' ? 1 : 0);
        ...
        Cache::put('setting:old', 'stale');
        Cache::put('setting:new', 'stale');
        Bus::fake();
