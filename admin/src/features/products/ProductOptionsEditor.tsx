...
export function ProductOptionsEditor({
  ...
  return <Card className="overflow-hidden">
    <div className="flex items-center justify-between gap-4 p-6">
      <div><h2 className="text-lg font-semibold">{t('products.options_variants')}</h2><p className="mt-1 text-sm text-slate-500">{expanded ? t('products.options_help') : t('products.options_variants_summary', { options: options.length, variants: variants.length })}</p></div>
      <Button type="button" variant="secondary" aria-expanded={expanded} onClick={() => setExpanded((current) => !current)}>{t('common.edit')}</Button>
    </div>
