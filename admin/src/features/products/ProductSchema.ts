...
export function productDefaults(product: ProductDetail): ProductFormValues { return { slug: product.slug ?? '', status: product.status, brand_id: product.brand_id, attributes: Object.fromEntries(product.product_attributes.map((a) => [a.handle, a.value])), collections: product.collection_ids, media: product.media }; }
export function productPayload(values: ProductFormValues): UpdateProductPayload { return { slug: values.slug.trim().toLowerCase(), status: values.status, brand_id: values.brand_id, attributes: values.attributes, collections: values.collections, media: values.media.map((media) => ({ id: Number(media.id), source: media.source, alt: media.alt })) }; }
