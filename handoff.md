...
**One real, unrelated bug found and fixed locally (NOT yet deployed)**: `/blog`'s category filter ("Breed Guides" tab) showed "No stories found" even with a real matching post. Root cause: `BlogPage.tsx`'s `featuredPost` was computed from the unfiltered post list while `latestPosts` excluded that post's id from the *filtered* list — when the globally-newest post was also the only post in the selected category, it disappeared from the grid entirely. Fixed by deriving both from the same filtered base. See `RULES.md` for the general pattern.
...
2. **Deploy the `BlogPage.tsx` fix** — commit + push + `git pull` + rebuild frontend + `--force-recreate` on the VPS. Small, isolated change; safe to ship on its own.
