...
- **Fulfillment status and payment status are deliberately separate fields** — `status` (the state machine above) never becomes `"refunded"`; that's tracked independently in `meta.payment_status` (`paid` / `partially-refunded` / `refunded`, set by `OrderOperationsService::refundOrder()`). This is intentional: a `delivered` order that gets refunded is still `delivered` (it happened), not retroactively un-delivered — conflating the two into one badge would lose that.
